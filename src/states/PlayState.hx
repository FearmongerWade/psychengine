...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var showTime:Bool = (Settings.data.timeBarType != 'Disabled');
		...
		if(Settings.data.timeBarType == 'Song Name')
		{
			timeTxt.size = 24;
			timeTxt.y += 3;
		}
		...
		healthBar.visible = !Settings.data.hideHud;
		...
		iconP1.visible = !Settings.data.hideHud;
		iconP1.alpha = Settings.data.healthBarAlpha;
		uiGroup.add(iconP1);

		iconP2 = new HealthIcon(dad.healthIcon, false);
		iconP2.y = healthBar.y - 75;
		iconP2.visible = !Settings.data.hideHud;
		...
		else if(Paths.formatToSongPath(Settings.data.pauseMusic) != 'none')
			Paths.music(Paths.formatToSongPath(Settings.data.pauseMusic));
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			for (i in 0...opponentStrums.length) {
				...
				//if(Settings.data.middleScroll) opponentStrums.members[i].visible = false;
			}
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				var introAlts:Array<String> = introAssets.get(stageUI);
				var antialias:Bool = (Settings.data.antialiasing && !isPixelStage);
				...
				if(!skipArrowStartTween)
				{
					notes.forEachAlive(function(note:Note) {
						if(Settings.data.opponentStrums || note.mustPress)
						{
							...
							if(Settings.data.middleScroll && !note.mustPress)
								note.alpha *= 0.35;
						}
					});
				}
			...
			for (i in 0...section.sectionNotes.length)
			{
				...
				if(roundSus > 0)
				{
					for (susNote in 0...roundSus)
					{
						...
						else if(Settings.data.middleScroll)
						{
							sustainNote.x += 310;
	...
	{
		var strumLineX:Float = Settings.data.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X;
		var strumLineY:Float = Settings.data.downScroll ? (FlxG.height - 150) : 50;
		for (i in 0...4)
		{
			// FlxG.log.add(i);
			...
			if (player < 1)
			{
				...
				else if(Settings.data.middleScroll) targetAlpha = 0.35;
			}

			var babyArrow:StrumNote = new StrumNote(strumLineX, strumLineY, i, player);
			babyArrow.downScroll = Settings.data.downScroll;
	...
	function resetRPC(?showTime:Bool = false)
	{
		...
		if (showTime)
			DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter(), true, songLength - Conductor.songPosition - Settings.data.noteOffset);
		else
	...
	override public function update(elapsed:Float)
	{
		...
		if (!Settings.data.noReset && controls.RESET && canReset && !inCutscene && startedCountdown && !endingSong)
		{
			health = 0;
			trace("RESET = True");
		}
		...
		switch(eventName) {
			...
			case 'Change Scroll Speed':
				if (songSpeedType != "constant")
				{
					...
					var newValue:Float = SONG.speed * Settings.getGameplaySetting('scrollspeed') * flValue1;
					if(flValue2 <= 0)
						songSpeed = newValue;
					else
	...
	public function finishSong(?ignoreNoteOffset:Bool = false):Void
	{
		...
		if(Settings.data.noteOffset <= 0 || ignoreNoteOffset) {
			endCallback();
		} else {
			finishTimer = new FlxTimer().start(Settings.data.noteOffset / 1000, function(tmr:FlxTimer) {
				endCallback();
			});
		}
	}
	...
	{
		var noteDiff:Float = Math.abs(note.strumTime - Conductor.songPosition + Settings.data.ratingOffset);
		vocals.volume = 1;

		if (!Settings.data.comboStacking && comboGroup.members.length > 0)
		{
		...
		for (i in 0...separatedScore.length)
		{
			...
			numScore.visible = !Settings.data.hideHud;
	...
	override function beatHit()
	{
		...
		if (generatedMusic)
			notes.sort(FlxSort.byY, Settings.data.downScroll ? FlxSort.ASCENDING : FlxSort.DESCENDING);
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			if (camZooming && FlxG.camera.zoom < 1.35 && Settings.data.camZooms)
			{
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null)
	{
		...
		var usedPractice:Bool = (Settings.getGameplaySetting('practice') || Settings.getGameplaySetting('botplay'));
